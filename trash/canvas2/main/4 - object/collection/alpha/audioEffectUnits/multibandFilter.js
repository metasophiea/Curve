this.multibandFilter = function(
    ...
    //main object
        var object = canvas.object.builder(this.multibandFilter,design);
    ...
    //circuitry
        object.filterCircuit_0 = new canvas.part.circuit.audio.multibandFilter(canvas.library.audio.context, vars.channelCount, true);
        object.filterCircuit_1 = new canvas.part.circuit.audio.multibandFilter(canvas.library.audio.context, vars.channelCount, true);
    ...
    //internal functions
        function getFrequencyAndLocationArray(){
            ...
            return {frequency:frequencyArray, location:canvas.library.math.normalizeStretchArray(locationArray)};
        }
    ...
    //wiring
        ...
        for(var a = 0; a < vars.channelCount; a++){
            ...
            object.elements.dial_continuous['qDial_'+a].onchange = function(a){
                return function(value){
                    vars.Q[a] = value;
                    object.filterCircuit_0.Q(a, canvas.library.math.curvePoint.exponential(vars.Q[a],0,20000,vars.curvePointExponentialSharpness));
                    object.filterCircuit_1.Q(a, canvas.library.math.curvePoint.exponential(vars.Q[a],0,20000,vars.curvePointExponentialSharpness));
                    updateGraph(a);
                }
            }(a);
            object.elements.dial_continuous['frequencyDial_'+a].onchange = function(a){
                return function(value){
                    ...
                    object.elements.readout_sixteenSegmentDisplay['frequencyReadout_'+a].text( canvas.library.misc.padString( canvas.library.math.curvePoint.exponential(value,0,20000,vars.curvePointExponentialSharpness).toFixed(2), 8) );
                    object.elements.readout_sixteenSegmentDisplay['frequencyReadout_'+a].print('smart');
                    object.filterCircuit_0.frequency(a, canvas.library.math.curvePoint.exponential(vars.frequency[a],0,20000,vars.curvePointExponentialSharpness));
                    object.filterCircuit_1.frequency(a, canvas.library.math.curvePoint.exponential(vars.frequency[a],0,20000,vars.curvePointExponentialSharpness));
                    updateGraph(a);
                }
            }(a);
        }
