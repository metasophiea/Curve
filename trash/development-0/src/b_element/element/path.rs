    ...
    //attributes
        //pertinent to extremity calculation
            //points
                ...
                pub fn set_points(&mut self, new:Polygon) { self.points = new; self.calculate_points(); self.compute_extremities(true, None); }
            //thickness
                pub fn get_thickness(&self) -> &f32 { &self.thickness }
                pub fn set_thickness(&mut self, new:f32) { self.thickness = new; self.calculate_points(); self.compute_extremities(true, None); }
            //joint_detail
                pub fn get_joint_detail(&self) -> &u32 { &self.joint_detail }
                pub fn set_joint_detail(&mut self, new:u32) { self.joint_detail = new; self.calculate_points(); self.compute_extremities(true, None); }
            //joint_type
                pub fn get_joint_type(&self) -> &PathJointType { &self.joint_type }
                pub fn set_joint_type(&mut self, new:PathJointType) { self.joint_type = new; self.calculate_points(); self.compute_extremities(true, None); }
            //sharp_limit
                pub fn get_sharp_limit(&self) -> &f32 { &self.sharp_limit }
                pub fn set_sharp_limit(&mut self, new:f32) { self.sharp_limit = new; self.calculate_points(); self.compute_extremities(true, None); }
            //cap_type
                pub fn get_cap_type(&self) -> &PathCapType { &self.cap_type }
                pub fn set_cap_type(&mut self, new:PathCapType) { self.cap_type = new; self.calculate_points(); self.compute_extremities(true, None); }
            //loop_path
                pub fn get_loop_path(&self) -> &bool { &self.loop_path }
                pub fn set_loop_path(&mut self, new:bool) { self.loop_path = new; self.calculate_points(); self.compute_extremities(true, None); }
        //other
