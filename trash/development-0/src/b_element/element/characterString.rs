...
impl CharacterString {
    ...
    //unified attribute
        pub fn set_unified_attribute(
            ...
            if let Some(x) = x { self.inner_group.set_x(x); }
            if let Some(y) = y { self.inner_group.set_y(y); }
            if let Some(angle) = angle { self.inner_group.set_angle(angle); }
            if let Some(scale) = scale { self.inner_group.set_scale(scale); }
            ...
            if let Some(colour) = colour { 
                ...
                for child in &self.children {
                    child.borrow_mut().as_character_mut().unwrap().set_colour( self.colour );
                }
            }
            ...
            if calculate_points {
                ...
                self.compute_extremities(true, Some(&self.get_parent_offset()));
            }
        }

    //string
        fn generate_string_characters(&mut self, font_requester:&RefCell<FontRequester>, worker:&web_sys::Worker) {
            self.inner_group.clear();
            self.children.clear();
            ...
            //create character and add it to the group
                for character in self.string.chars() {
                    ...
                    self.inner_group.__direct_append(weak_ref);
                }
            ...
            //printing_mode_horizontal
                if self.printing_mode_horizontal == PrintingModePositionHorizontal::Middle { 
                    for child in &self.children {
                        ...
                        child.borrow_mut().set_x( x - cumulative_width/2.0 );
                    }
                } else if self.printing_mode_horizontal == PrintingModePositionHorizontal::Right {
                    for child in &self.children {
                        let x = child.borrow().get_x();
                        child.borrow_mut().set_x( x - cumulative_width );
                    }
                }
...
impl ElementTrait for CharacterString {
    //trait requirements
        ...
        //position
            //x
                ...
                fn set_x(&mut self, new:f32){ self.inner_group.set_x(new); }
            //y
                fn get_y(&self) -> f32 { self.inner_group.get_y() }
                fn set_y(&mut self, new:f32){ self.inner_group.set_y(new); }
            //angle
                fn get_angle(&self) -> f32 { self.inner_group.get_angle() }
                fn set_angle(&mut self, new:f32) { self.inner_group.set_angle(new); }
            //scale
                fn get_scale(&self) -> f32 { self.inner_group.get_scale() }
                fn set_scale(&mut self, new:f32) { self.inner_group.set_scale(new); }
        ...
        //extremities
            fn get_allow_compute_extremities(&self) -> bool { self.inner_group.get_allow_compute_extremities() }
            fn set_allow_compute_extremities(&mut self, new:bool) { self.inner_group.set_allow_compute_extremities(new); }
    ...
    //element specific
        ...
        //point calculation
            fn calculate_points(&mut self, font_requester:&RefCell<FontRequester>, worker:&web_sys::Worker) {
                self.generate_string_characters(font_requester, worker);
            }

        //extremities
            fn compute_extremities(&mut self, inform_parent:bool, offset:Option<&Offset>) {
                self.inner_group.compute_extremities(false, offset);
        ...
        //render
            fn should_this_element_render(&self, parent_clipping_polygon:Option<&Polygon>, viewbox:&Viewbox, heed_camera:Option<bool>) -> bool {
                self.inner_group.should_this_element_render(parent_clipping_polygon, viewbox, heed_camera)
            }
            fn render(
                &mut self,
