...
impl Arrangement {
    ...
    //root
        ...
        pub fn prepend(&self, element_manager:&mut ElementManager, new_element_id:usize) {
            element_manager.get_element_by_id_mut(self.root_element_id).unwrap().as_group_mut().unwrap().prepend(&element_manager,new_element_id);
        }
        pub fn append(&self, element_manager:&mut ElementManager, new_element_id:usize) {
            element_manager.get_element_by_id_mut(self.root_element_id).unwrap().as_group_mut().unwrap().append(&element_manager,new_element_id);
        }
        pub fn remove(&self, element_manager:&mut ElementManager, departing_element:usize) {
            element_manager.get_element_by_id_mut(self.root_element_id).unwrap().as_group_mut().unwrap().remove(&element_manager,departing_element);
        }
        pub fn clear(&self, element_manager:&mut ElementManager) {
            element_manager.get_element_by_id_mut(self.root_element_id).unwrap().as_group_mut().unwrap().clear();
        }
    ...
    //misc
        pub fn print_tree(&self, element_manager:&ElementManager, mode:Option<u8>) { //modes: 0:spaced / 1:tabular / 2:address
            ...
            fn get_the_facts(element:&Ref<dyn ElementTrait>) -> String {
                format!(
                    "{} (id:{}, type:{}, x:{}, y:{}, angle:{}, scale:{}, heed_camera:{})",
                    element.get_name(),
                    element.get_id(),
                    ...
                    if element.get_parent_id() != Some(0) || element.get_element_type() != ElementType::Group { 
                        ...
                        if element.as_group().unwrap().get_heed_camera() { "true" } else { "false" }
                    }
                )
            }
    ...
    impl Engine {
        //root
            pub fn arrangement__prepend(&mut self, element_id:usize) {
                self.arrangement.prepend(&mut self.element_manager, element_id);
            }
            pub fn arrangement__append(&mut self, element_id:usize) {
                self.arrangement.append(&mut self.element_manager, element_id);
            }
            pub fn arrangement__remove(&mut self, element_id:usize) {
                self.arrangement.remove(&mut self.element_manager, element_id);
            }
            pub fn arrangement__clear(&mut self) {
                self.arrangement.clear(&mut self.element_manager);
            }
        ...
        //discovery
            ...
            //     self.arrangement.get_elements_under_point(&self.element_manager, None, Point::new(x,y))
            // }
            // pub fn arrangement__get_elements_under_area(&self, polygon:Vec<f32>) -> Vec<usize> {
            //     self.arrangement.get_elements_under_area(&self.element_manager, None, Polygon::new_from_flat_array(polygon))
            // }
