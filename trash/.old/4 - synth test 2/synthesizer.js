parts.audio.synthesizer_basic = function(
    ...
    function makeOSC(
        ...
        return new function(){
            this.generator = context.createOscillator();
                ...
                this.generator.frequency.value = __globals.audio.num2freq(midiNumber,octave);
    ...
    this.perform = function(note){
        if( !liveOscillators[note.number] && note.velocity == 0 ){/*trying to stop a non-existant tone*/return;}
        else if( !liveOscillators[note.number] ){ 
            //create new tone
            liveOscillators[note.number] = makeOSC(context, mainOut, note.number, type, periodicWave, note.velocity, attack, release, detune, octave); 
        }
        else if( note.velocity == 0 ){ 
            //stop and destroy tone
            liveOscillators[note.number].stop();
            delete liveOscillators[note.number];
        }
        else{
            //adjust tone
            liveOscillators[note.number].changeVelocity(note.velocity);
        }
    };
    this.panic = function(){
        var OSCs = Object.keys(liveOscillators);
        for(var a = 0; a < OSCs.length; a++){ this.perform( {'number':OSCs[a], 'velocity':0} ); }
    };
};