    ...
    //attributes
        //pertinent to extremity calculation
            //points
                ...
                pub fn set_points(&mut self, new:Polygon) { self.points = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //thickness
                pub fn get_thickness(&self) -> &f32 { &self.thickness }
                pub fn set_thickness(&mut self, new:f32) { self.thickness = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //joint_detail
                pub fn get_joint_detail(&self) -> &u32 { &self.joint_detail }
                pub fn set_joint_detail(&mut self, new:u32) { self.joint_detail = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //joint_type
                pub fn get_joint_type(&self) -> &PathJointType { &self.joint_type }
                pub fn set_joint_type(&mut self, new:PathJointType) { self.joint_type = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //sharp_limit
                pub fn get_sharp_limit(&self) -> &f32 { &self.sharp_limit }
                pub fn set_sharp_limit(&mut self, new:f32) { self.sharp_limit = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //cap_type
                pub fn get_cap_type(&self) -> &PathCapType { &self.cap_type }
                pub fn set_cap_type(&mut self, new:PathCapType) { self.cap_type = new; self.calculate_points(); self.compute_extremities(true, None, None); }
            //loop_path
                pub fn get_loop_path(&self) -> &bool { &self.loop_path }
                pub fn set_loop_path(&mut self, new:bool) { self.loop_path = new; self.calculate_points(); self.compute_extremities(true, None, None); }
        //other
        ...
        //unified attribute
            pub fn set_unified_attribute(
                ...
                if let Some(thickness) = thickness { self.thickness = thickness; self.calculate_points(); }
                if let Some(joint_detail) = joint_detail { self.joint_detail = joint_detail; self.calculate_points(); }
                if let Some(joint_type) = joint_type { self.joint_type = joint_type; self.calculate_points(); }
                if let Some(sharp_limit) = sharp_limit { self.sharp_limit = sharp_limit; self.calculate_points(); }
                if let Some(cap_type) = cap_type { self.cap_type = cap_type; self.calculate_points(); }
                if let Some(loop_path) = loop_path { self.loop_path = loop_path; self.calculate_points(); }
