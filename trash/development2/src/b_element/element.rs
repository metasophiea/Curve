...
pub trait ElementTrait {
    ...
    //extremities
        fn should_heed_camera(&self) -> bool {
            ...
            match &self.get_parent() {
                None => false,
                Some(parent) => parent.upgrade().unwrap().borrow().should_heed_camera(),
            }
        }
        ...
        fn compute_extremities(&mut self, inform_parent:bool, offset:Option<&Offset>, _heed_camera:Option<bool>) {
    ...
    //render
        ...
        fn render(
            ...
            //calculate offset for this element
                let storage; //so that the created offset data can live while only being borrowed
                let working_offset = if heed_camera {
                    storage = Offset::combine(
    ...
    //info/dump
        ...
        fn _info(&self) -> String {
            format!(
                "{{element_type:{}, id:{}, name:\"{}\", parent_id:{}, parent_name:\"{}\", x:{}, y:{}, angle:{}, scale:{}, ignored:{}, cached_offset:{}, {}, dot_frame:{}, allow_compute_extremities:{}, extremities:{}}}",
