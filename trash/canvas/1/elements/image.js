this.image = {
    ...
    draw:function(context,x,y,width,height,angle=0,anchor={x:0,y:0},url,static=false){
        ...
        //if this image url is not cached; cache it
            if( !elementLibrary.image.cache.hasOwnProperty(url) ){
                elementLibrary.image.cache[url] = new Image(); 
                elementLibrary.image.cache[url].src = url;
            }
    
        //actual render (using cached image)
            context.save();
            context.rotate( position.angle );
            context.drawImage( elementLibrary.image.cache[url], position.location.x, position.location.y, width, height );
            context.restore();
    },
