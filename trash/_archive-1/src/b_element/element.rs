...
pub trait ElementTrait {
    ...
    //render
        fn get_ancestor_clipping_polygon(&self) -> Option<data_type::Polygon> {
            match &self.get_parent() {
                None => None,
                Some(parent) => {
                    if self.get_element_type() != ElementType::Group {
                        parent.upgrade().unwrap().borrow().get_ancestor_clipping_polygon()
                    } else { //you are a group
                        if self.as_group().unwrap().get_clip_active() {
                            Some(self.get_extremities().clone())
                        } else {
                            parent.upgrade().unwrap().borrow().get_ancestor_clipping_polygon()
                        }
                    }
        ...
        fn should_heed_camera(&self) -> bool {
            match &self.get_parent() {
                None => false,
                Some(parent) => {
                    if self.get_element_type() != ElementType::Group {
                        parent.upgrade().unwrap().borrow().should_heed_camera()
                    } else if self.get_parent_id().unwrap() != 0 { //you are a group
                        parent.upgrade().unwrap().borrow().should_heed_camera()
                    } else { //you are a group, you are a child of the root
                        self.as_group().unwrap().get_heed_camera()
                    }
                },
            }
        }
        fn inform_parent_of_change(&mut self, viewbox:&Viewbox) {
            if !self.should_this_element_render(self.get_ancestor_clipping_polygon().as_ref(), viewbox, self.should_heed_camera()) {
                return;
            }
            ...
            match &self.get_parent() {
                None => {}
                Some(parent) => {
                    match parent.upgrade().unwrap().borrow_mut().as_group_mut(){
                        None => {},
                        Some(group_parent) => {
                            group_parent.child_changed(Some(self.get_id()), viewbox);
                        },
                    }
                },
            }
        }
        fn should_this_element_render(&self, parent_clipping_polygon:Option<&data_type::Polygon>, viewbox:&Viewbox, heed_camera:bool) -> bool {
        ...
        fn render(
            ...
            //true - this element needs to be re-rendered next frame
            //false - this element rendered successfully 
            ...
            //activate shape render code
                let result = self.activate_webGL_render(
                    &adjusted_offset,
                    context, 
