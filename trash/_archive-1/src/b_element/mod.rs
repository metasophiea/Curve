...
impl ElementManager {
    ...
    pub fn delete(&mut self, id:usize, viewbox:&Viewbox) -> Result<(),String> {
        ...
        match parent_id {
            None => {},
            Some(parent_id) => self.get_element_by_id_mut(parent_id).unwrap().as_group_mut().unwrap().remove(self, id, viewbox),
        }
...
impl ElementManager {
    pub fn create_set_append(
        ...
        //set
            self.get_element_by_id_mut(id).unwrap().set_unified_attribute_from_js_sys_object(universal_attribute, &self.font_requester, worker, viewbox);

        //append
            self.execute_method__Group__append(group_id, id, viewbox);
    ...
    pub fn draw_dot(
        ...
        if !heed_camera {
            rect.render( None, parent_clipping_polygon, heed_camera, &Offset::new_default(), viewbox, context, web_gl2_program_conglomerate_manager, web_gl2_framebuffer_manager, image_requester, resolution, true );
        } else {
            rect.render( None, parent_clipping_polygon, heed_camera, offset, viewbox, context, web_gl2_program_conglomerate_manager, web_gl2_framebuffer_manager, image_requester, resolution, true );
        }
    }
    pub fn alert_font_loaded(
        ...
        for id in ids.iter() {
            match self.get_element_by_id_mut(*id) {
                Some(mut element) => element.calculate_points(&self.font_requester, worker, viewbox),
...
//engine // basic management
    ...
    impl Engine {
        ...
        pub fn element__delete(&mut self, id:usize) -> bool {
            match self.element_manager.delete(id, self.viewport.get_viewbox()) {
                Ok(_) => true,
...
//engine // misc
    ...
    impl Engine {
        pub fn element__create_set_append(
            ...
            self.element_manager.create_set_append( type_string, name, universal_attribute, group_id, &self.worker, self.viewport.get_viewbox() )
        }
        pub fn element__alert_font_loaded(
            ...
            self.element_manager.alert_font_loaded( &font_name, load_was_success, &self.worker, self.viewport.get_viewbox() );
        }
