    ...
    //unified attribute
        pub fn set_unified_attribute(
            ...
            if calculate_points {
                ...
                self.compute_extremities(true, Some(&self.get_parent_offset()), viewbox);
            }
        }
...
impl ElementTrait for CharacterString {
    //trait requirements
        ...
        //position
            //x
                ...
                fn set_x(&mut self, new:f32, viewbox:&Viewbox){ self.inner_group.set_x(new, viewbox); self.compute_extremities(true, None, viewbox); }
            //y
                fn get_y(&self) -> f32 { self.inner_group.get_y() }
                fn set_y(&mut self, new:f32, viewbox:&Viewbox){ self.inner_group.set_y(new, viewbox); self.compute_extremities(true, None, viewbox); }
            //angle
                fn get_angle(&self) -> f32 { self.inner_group.get_angle() }
                fn set_angle(&mut self, new:f32, viewbox:&Viewbox) { self.inner_group.set_angle(new, viewbox); self.compute_extremities(true, None, viewbox); }
            //scale
                fn get_scale(&self) -> f32 { self.inner_group.get_scale() }
                fn set_scale(&mut self, new:f32, viewbox:&Viewbox) { self.inner_group.set_scale(new, viewbox); self.compute_extremities(true, None, viewbox); }
    ...
    //element specific
        ...
        //universal attribute
            fn set_unified_attribute_from_js_sys_object(&mut self, unified_attribute:js_sys::Object, font_requester:&RefCell<FontRequester>, worker:&web_sys::Worker, viewbox:&Viewbox) {
        ...
        //extremities
            fn compute_extremities(&mut self, inform_parent:bool, offset:Option<&Offset>, viewbox:&Viewbox) {
                self.inner_group.compute_extremities(false, offset, viewbox);
        ...
        //render
            fn should_this_element_render(&self, parent_clipping_polygon:Option<&Polygon>, viewbox:&Viewbox, heed_camera:bool) -> bool {
            ...
            fn render(
                ...
                let result = self.inner_group.render(
                    parent_id,
