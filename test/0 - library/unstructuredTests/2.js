...
//     osc.connect(gain).connect(_canvas_.library.audio.context.destination);
//     gain.connect(MAM);


//     let MAMvalue = 0;
//     MAM.parameters.get('updateDelay').setValueAtTime(50,0);
//     MAM.parameters.get('fullSample').setValueAtTime(1,0);
//     MAM.port.onmessage = function(msg){ MAMvalue = msg.data };
...
//     const frequencyJourney = _canvas_.library.math.curveGenerator.linear(100,100,1000).map(a => Math.floor(a));
//     let step = 0;

//     const resultArray = [];
//     const interval = setInterval(() => {
//         // osc.frequency.setTargetAtTime(frequencyJourney[step], _canvas_.library.audio.context.currentTime, 0);
//         gain.gain.setTargetAtTime(frequencyJourney[step]/1000, _canvas_.library.audio.context.currentTime, 0);

//         setTimeout(() => {
//             console.log( step, frequencyJourney[step], MAMvalue );
//             resultArray.push(MAMvalue);

//             step++;
//             if(step >= frequencyJourney.length){
//                 clearInterval(interval);
//                 console.log( resultArray.map(a => Math.floor(a*100)/100) );
//             }
//         },75);
//     },100);
// });
