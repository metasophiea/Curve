...
var unit_4;
function resetScene(){
    _canvas_.control.scene2.new();
    _canvas_.control.selection.clearClipboard();
    _canvas_.control.selection.deselectEverything();
    unit_1 = _canvas_.control.scene2.addUnit(10,10,0,'testUnit_2','test');
    unit_2 = _canvas_.control.scene2.addUnit(200,10,0.1,'testUnit_2','test');
    unit_3 = _canvas_.control.scene2.addUnit(10,150,0,'testUnit_3','test');
    unit_4 = _canvas_.control.scene2.addUnit(150,150,0,'testUnit_3','test');
...
tester(_canvas_.control.scene2.getAllUnits().length,3);
tester(
    _canvas_.control.scene2.documentUnits( _canvas_.control.scene2.getAllUnits().filter(a => !a._isCable) ),
    [
        {
...
_canvas_.control.selection.cut();
tester(_canvas_.control.scene2.getAllUnits().length,1);
tester(
    _canvas_.control.selection.clipboard,
    [
        {
...
tester(_canvas_.control.scene2.getAllUnits().length,4);
tester(
    _canvas_.control.scene2.documentUnits( _canvas_.control.scene2.getAllUnits().filter(a => !a._isCable) ),
    [
        {position:{x:200,y:10,angle:0.1},details:{collection:"test",model:"testUnit_2"},data:{},connections:[]},
...
tester(_canvas_.control.scene2.getAllUnits().length,5);
tester(
    _canvas_.control.scene2.documentUnits( _canvas_.control.scene2.getAllUnits().filter(a => !a._isCable) ),
    [        
        {position:{x:200,y:10,angle:0.1},details:{collection:"test",model:"testUnit_2"},data:{},connections:[]},
