...
pub struct Group {
    ...
    //render
        ...
        cached_rendering_offset: Offset,
...
impl Group {
    pub fn new(id:usize, name: String) -> Group {
        Group {
            ...
            cached_rendering_offset: Offset::new_default(),
    ...
    //attributes
        //pertinent to extremity calculation
            //heed_camera
                ...
                pub fn set_heed_camera(&mut self, new:bool, viewbox:&Viewbox) { self.heed_camera = new; self.compute_extremities(true, None, viewbox); }
            //framebuffer_active
            ...
            //unified attribute
                pub fn set_unified_attribute(
                    ...
                    self.compute_extremities(true, None, viewbox);
                }
    ...
    //group functions
        ...
        pub fn append(&mut self, em:&ElementManager, new_element:usize, viewbox:&Viewbox) {
            ...
            //inform parent renderer of change
                self.child_changed(Some(new_element), viewbox);
        }
        pub fn __direct_append(&mut self, new_element:Weak<RefCell<dyn ElementTrait>>, viewbox:&Viewbox) {
            ...
            //inform parent renderer of change
                self.child_changed(None, viewbox);
        }
        ...
        pub fn remove(&mut self, em:&ElementManager, departing_element:usize, viewbox:&Viewbox){
            ...
            //inform parent renderer of change (before the removal, just incase this group used to be in the viewable area, but with this removal it no longer is)
                self.child_changed(None, viewbox);
        ...
        pub fn clear(&mut self, viewbox:&Viewbox) {
            //inform parent renderer of change (before the removal, just incase this group used to be in the viewable area, but with this clearing it no longer is)
                self.child_changed(None, viewbox);
        ...
        pub fn shift(&mut self, element_to_shift:usize, mut new_position:usize, viewbox:&Viewbox) {
            ...
            for new_element in new_elements {
                //validate 
                    if new_element == 0 {
                        ...
                        //It doesn't have a parent, thus there is no group to shift it within. As such, zero can be assumed to be an 'undefined' value
    ...
    //extremity calculation
        ...
        fn augment_extremities__add(&mut self, new_element:&Weak<RefCell<dyn ElementTrait>>, viewbox:&Viewbox) {
            ...
            //run computeExtremities on new child
                new_element.upgrade().unwrap().borrow_mut().compute_extremities(false, Some(&adjusted_offset), viewbox);
            //augment points list
    ...
    //render
        pub fn child_changed(&mut self, _child_id:Option<usize>, viewbox:&Viewbox){
            // _child_id : None = can't be sure what child changed
            self.children_changed = true;
            self.inform_parent_of_change(viewbox);
        }
...
impl ElementTrait for Group {
    //trait requirements
        ...
        //position
            //x
                ...
                fn set_x(&mut self, new:f32, viewbox:&Viewbox){ self.x = new; self.compute_extremities(true, None, viewbox); }
            //y
                fn get_y(&self) -> f32 { self.y }
                fn set_y(&mut self, new:f32, viewbox:&Viewbox){ self.y = new; self.compute_extremities(true, None, viewbox); }
            //angle
                fn get_angle(&self) -> f32 { self.angle }
                fn set_angle(&mut self, new:f32, viewbox:&Viewbox) { self.angle = new; self.compute_extremities(true, None, viewbox); }
            //scale
                fn get_scale(&self) -> f32 { self.scale }
                fn set_scale(&mut self, new:f32, viewbox:&Viewbox) { self.scale = new; self.compute_extremities(true, None, viewbox); }
    ...
    //element specific
        ...
        //universal attribute
            fn set_unified_attribute_from_js_sys_object(&mut self, unified_attribute:js_sys::Object, _font_requester:&RefCell<FontRequester>, _worker:&web_sys::Worker, viewbox:&Viewbox) {
        ...
        //extremities
            fn compute_extremities(&mut self, inform_parent:bool, offset:Option<&Offset>, viewbox:&Viewbox) {
                ...
                //run computeExtremities on all children
                    for child in self.children.iter() {
                        child.upgrade().unwrap().borrow_mut().compute_extremities(false, Some(&adjusted_offset), viewbox)
                    }
                //run computeExtremities on stencil (if applicable)
                    match &self.clipping_stencil {
                        None => {},
                        Some(stencil) => {
                            stencil.upgrade().unwrap().borrow_mut().compute_extremities(false, Some(&adjusted_offset), viewbox);
                        },
                    }
                //update extremities
        ...
        //render
            fn render(
                ...
                //judge whether this element should be allowed to render
                    if !force && self.get_id() != 0 && !self.should_this_element_render(parent_clipping_polygon, viewbox, heed_camera) {
                        return false;
                    }
                ...
                //if this element is heeding the camera; combine offset with shape's position, angle and scale to produce adjust value for render
                    let is_changed = if self.id == 0 {
                        self.cached_rendering_offset.update_return_is_changed(self.x, self.y, self.scale, self.angle)
                    } else if child_of_the_root && !heed_camera {
                        self.cached_rendering_offset.update_return_is_changed(self.x, self.y, self.scale, self.angle)
                    } else if child_of_the_root && heed_camera {
                        self.cached_rendering_offset.update_with_offset_return_is_changed(viewbox.get_offset())
                    } else {
                        self.cached_rendering_offset.update_with_offset_return_is_changed(
                            Offset::combine(
                                &Offset::new(self.x, self.y, self.scale, self.angle),
                                offset,
                            )
                    ...
                    //additional calculation for the root group (which must pass viewport offsets through to its children)
                        let is_changed = if self.id == 0 {
                            &self.cached_rendering_offset != offset
                        } else {
                            is_changed
                        };
                ...
                //framebuffer
                    if self.framebuffer_active {
                        if self.id == 0 { //only if this is root; activate framebuffer
                            //derender - if no children have reported a change, and the offset is no different, then don't render at all
                            if !self.children_changed && !is_changed && !force {
                ...
                //update parent_bounding_box is necessary
                    let parent_bounding_box = if self.clipping_active { 
                        Some(&self.extremities) 
                    } else {
                ...
                //render children
                    for child in self.children.iter() {
                        self.children_changed |= child.upgrade().unwrap().borrow_mut().render(
                            Some(&self.id),
                            parent_bounding_box,
                            heed_camera,
                            &self.cached_rendering_offset,
            ...
            fn draw_dot_frame(
                ...
                //draw bounding box top left and bottom right points
                    ElementManager::draw_dot(
                        ...
                        parent_clipping_polygon, heed_camera, &viewbox.get_offset(), viewbox, context, web_gl2_program_conglomerate_manager, web_gl2_framebuffer_manager, image_requester, resolution
                    );
                    ElementManager::draw_dot(
        ...
        //info/dump
            fn _specific_info(&self) -> String {
                ...
                format!(
                    "heed_camera:{}, children:{}, clipping_stencil_id:{}, clipping_stencil_name:{}, clipping_active:{}, framebuffer_active:{}, children_changed:{}, cached_rendering_offset:{}, web_gl2_framebuffer_id:{:?}",
                    self.heed_camera,
