    ...
    //attributes
        //pertinent to extremity calculation
            //radius
                ...
                pub fn set_radius(&mut self, new:f32, viewbox:&Viewbox) { self.radius = new; self.compute_extremities(true, None, viewbox); }
            //detail
                pub fn get_detail(&self) -> &u32 { &self.detail }
                pub fn set_detail(&mut self, new:u32, viewbox:&Viewbox) { self.detail = new; self.calculate_circle_points(); self.compute_extremities(true, None, viewbox); }
            //thickness
