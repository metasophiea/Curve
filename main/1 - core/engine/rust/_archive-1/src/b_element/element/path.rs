...
impl Path {
    ...
    //attributes
        //pertinent to extremity calculation
            //points
                ...
                pub fn set_points(&mut self, new:Polygon, viewbox:&Viewbox) { self.points = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //thickness
                pub fn get_thickness(&self) -> &f32 { &self.thickness }
                pub fn set_thickness(&mut self, new:f32, viewbox:&Viewbox) { self.thickness = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //joint_detail
                pub fn get_joint_detail(&self) -> &u32 { &self.joint_detail }
                pub fn set_joint_detail(&mut self, new:u32, viewbox:&Viewbox) { self.joint_detail = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //joint_type
                pub fn get_joint_type(&self) -> &PathJointType { &self.joint_type }
                pub fn set_joint_type(&mut self, new:PathJointType, viewbox:&Viewbox) { self.joint_type = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //sharp_limit
                pub fn get_sharp_limit(&self) -> &f32 { &self.sharp_limit }
                pub fn set_sharp_limit(&mut self, new:f32, viewbox:&Viewbox) { self.sharp_limit = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //cap_type
                pub fn get_cap_type(&self) -> &PathCapType { &self.cap_type }
                pub fn set_cap_type(&mut self, new:PathCapType, viewbox:&Viewbox) { self.cap_type = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
            //loop_path
                pub fn get_loop_path(&self) -> &bool { &self.loop_path }
                pub fn set_loop_path(&mut self, new:bool, viewbox:&Viewbox) { self.loop_path = new; self.calculate_points(viewbox); self.compute_extremities(true, None, viewbox); }
        //other
            pub fn get_colour(&self) -> &Colour { &self.colour }
            pub fn set_colour(&mut self, new:Colour, viewbox:&Viewbox) { self.colour = new; self.inform_parent_of_change(viewbox); }
        //unified attribute
            pub fn set_unified_attribute(
                ...
                if let Some(points) = points { self.points = points; self.calculate_points(viewbox); }
                if let Some(thickness) = thickness { self.thickness = thickness; self.calculate_points(viewbox); }
                if let Some(joint_detail) = joint_detail { self.joint_detail = joint_detail; self.calculate_points(viewbox); }
                if let Some(joint_type) = joint_type { self.joint_type = joint_type; self.calculate_points(viewbox); }
                if let Some(sharp_limit) = sharp_limit { self.sharp_limit = sharp_limit; self.calculate_points(viewbox); }
                if let Some(cap_type) = cap_type { self.cap_type = cap_type; self.calculate_points(viewbox); }
                if let Some(loop_path) = loop_path { self.loop_path = loop_path; self.calculate_points(viewbox); }
    ...
    //webGL rendering functions
        ...
        fn calculate_points(&mut self, _viewbox:&Viewbox) {
