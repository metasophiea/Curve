...
impl ElementRenderDecisionEntry {
    pub fn _dump(&self, prefix:Option<&str>) {
        ...
        for (key, value) in self.decisions.iter() {
...
impl ElementRenderDecision {
    ...
    pub fn clear_data(&mut self) {
        for (_key, value) in &mut self.database_by_type {
            value.clear();
        }
    }
...
impl ElementRenderDecision {
    pub fn _dump(&self, prefix:Option<&str>) {
        ...
        console_log!("{}┌─ElementRenderDecision Dump─", prefix);
        for (key, value) in self.database_by_type.iter() {
            console_log!("{}│ ┌ {}", prefix, key);
            value._dump( Some(&format!("{}│ ", prefix)) );
        }
        console_log!("{}└────────────────", prefix);
    }
}
...
impl Stats {
    ...
    pub fn element_render_decision_clear_data(&mut self) {
        if !self.active { return }
        self.element_render_decision.clear_data();
    }
    pub fn element_render_decision_register_info(&mut self, id:usize, element_type:&ElementType, render_decision:RenderDecision) {
        if !self.active { return }
...
impl Stats {
    pub fn _dump(&self) {
        ...
        console_log!("│");
        self.element_render_decision._dump(Some("│ "));
        console_log!("└────────────");
    }
}
...
impl Engine {
    ...
    pub fn stats__element_render_decision_clear_data(&mut self) {
        self.stats.element_render_decision_clear_data();
    }
