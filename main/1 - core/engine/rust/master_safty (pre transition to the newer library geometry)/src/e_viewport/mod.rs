...
impl Viewport {
    ...
    //camera position
        pub fn position(&mut self, point:Point, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) { 
            self.position = point; 
            self.calculate_viewbox(canvas_size, element_manager, arrangement);
        }
        pub fn scale(&mut self, s:f32, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) { 
            self.scale = s; 
            self.calculate_viewbox(canvas_size, element_manager, arrangement);
        }
        pub fn angle(&mut self, a:f32, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) { 
            self.angle = a; 
            self.calculate_viewbox(canvas_size, element_manager, arrangement);
        }
        pub fn anchor(&mut self, point:Point, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) { 
            self.anchor = point; 
            self.calculate_viewbox(canvas_size, element_manager, arrangement);
        }
    ...
    //misc
        fn calculate_viewbox(&mut self, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) {
            ...
            arrangement.calculate_visibility(element_manager, &self.viewbox);
        }
        pub fn get_viewbox(&self) -> &Viewbox {
            &self.viewbox
        }
        pub fn refresh(&mut self, canvas_size:(u32,u32), element_manager:&mut ElementManager, arrangement:&Arrangement) {
            self.calculate_viewbox(canvas_size, element_manager, arrangement);
        }
        pub fn _dump(&self) {
