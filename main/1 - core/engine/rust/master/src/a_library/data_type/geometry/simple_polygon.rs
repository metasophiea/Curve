...
//calculation
    impl SimplePolygon {
        fn produce_lines(points:&Vec<Point>) -> Vec<Line> {
            if points.len() == 0 {
                return vec![];
            } else if points.len() == 1 {
                return vec![ Line::new_from_points_ref(&points[0],&points[0]) ];
            }

            let mut output:Vec<Line> = vec![Line::new_empty(); points.len()];

            for index_a in 0..points.len()-1 {
                output[index_a].update_by_ref(
                    &points[index_a],
                    &points[index_a+1],
                );
            }
            output[points.len()-1].update_by_ref(
                &points[points.len()-1],
                &points[0],
            );
...
//new
    impl SimplePolygon {
        pub fn new_from_point_vector(points:Vec<Point>) -> SimplePolygon {
            let bounding_box = BoundingBox::new_from_points(&points);
            let lines = SimplePolygon::produce_lines(&points);
...
//setters
    impl SimplePolygon {
        pub fn prepend_point(&mut self, point:Point) {
            ...
            self.bounding_box = BoundingBox::new_from_points(&self.points);
            self.lines = SimplePolygon::produce_lines(&self.points);
        }
    ...
    impl SimplePolygon {
        pub fn intersect_with_point(&self, point:&Point) -> PolySide {
            ...
            fn point_level_with_poly_point_checker(poly:&SimplePolygon, point:&Point, index_a:usize, index_b:usize) -> bool {
                ...
                if poly.points[index_a].get_y() != point.get_y() && poly.points[index_b].get_y() != point.get_y() {
                    return true;
                } else if poly.points[index_a].get_y() == point.get_y() {
                    ...
                    for index_offset in 1..(poly.points.len()/2 + 1) {
                        //forward test
                            if forward_position.is_none() {
                                let forward_index = if (index_a + index_offset) >= poly.points.len() {
                                    index_a + index_offset - poly.points.len()
                                } else {
                                    index_a + index_offset
                                };
                                if poly.points[forward_index].get_y() < poly.points[index_a].get_y() {
                                    //forward_index is lower than this point
                                    forward_position = Some(false);
                                } else if poly.points[forward_index].get_y() > poly.points[index_a].get_y() {
                        ...
                        //backward test
                            if backward_position.is_none() {
                                let backward_index = if (index_a as isize - index_offset as isize) < 0 {
                                    poly.points.len() + index_a - index_offset
                                } else {
                                    index_a - index_offset
                                };
                                if poly.points[backward_index].get_y() < poly.points[index_a].get_y() {
                                    //backward_index is lower than this point");
                                    backward_position = Some(false);
                                } else if poly.points[backward_index].get_y() > poly.points[index_a].get_y() {
        ...
        pub fn intersect_with_line(&self, line:&Line) -> LinePolygonIntersectionResult {
            ...
            //detection of a polygon with a single point
                if self.points.len() == 1 {
                    let contact = self.points[0].intersect_with_line(line) == LineSide::Upon;

                    return LinePolygonIntersectionResult {
                        points: if contact { vec![self.points[0]] } else { vec![] },
                        contact: contact,
                        intersect: false,
        ...
        //reduced functionality, for pure speed
        pub fn intersect_with_simple_polygon__intersect_only(&self, other:&SimplePolygon) -> bool {
            //clearly different polygons
                if !self.get_bounding_box().intersect_with_bounding_box(&other.get_bounding_box()) {
                    return false;
                }
            ...
            //also, if one side intersects and inverse_intersects; that's a traversal
                ...
                for a_b in 0..self.get_points_length() {
                    ...
                    output_result.contact |= tmp.contact;
                    output_result.intersect |= tmp.intersect; if output_result.intersect { return true; }
                    output_result.traverse |= tmp.traverse;
            ...
            //if necessary, check for intersect by seeing if any point of the other poly is within the self poly
                for point in other.get_points() {
                    if output_result.intersect { break; }
                    output_result.intersect |= self.intersect_with_point(&point) == PolySide::Inside; if output_result.intersect { return true; }
                }

            output_result.intersect
        }
    }