...
pub struct WebGl2programConglomerate {
...
pub struct WebGl2programConglomerateManager {
    ...
    currently_active_program: Option<ElementType>,
}
...
impl WebGl2programConglomerateManager {
    pub fn new() -> WebGl2programConglomerateManager {
        WebGl2programConglomerateManager {
            ...
            currently_active_program: None, //ElementType::Group, //(Group doesn't have a rendering program)
        }
    }
    ...
    pub fn __manually_load_program(
        &mut self, 
        context: &WebGl2RenderingContext, 
        element_type: Option<ElementType>, 
        program: &WebGl2programConglomerate,
        VAO_id: usize,
    ) {
    ...
    pub fn load_program(
    ...
    ) -> Option<WebGl2programConglomerate> {
        fn create_program_conglomerate(
            context: &WebGl2RenderingContext, 
            element_type: Option<ElementType>, 
        ...
        //generate program conglomerate if one hasn't been created already
            match element_type {
                Some(element_type) => {
                    match self.program_store.get(&element_type) {
                        ...
                        None => { //creating new program conglomerate
                            let program_conglomerate = match create_program_conglomerate(
                                context,
                                Some(element_type),
                ...
                None => { //creating temporary program conglomerate
                    //a temporary program conglomerate will not be stored, instead returned so that whoever called it can store 
                    //it themselves, and load it manually with the __manually_load_program method
                    create_program_conglomerate(
                        context,
                        None,
                        vertex_shader_source,
                        fragment_shader_source,
