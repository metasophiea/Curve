...
fontFileNames.forEach(name => {
    ...
    const libraryEntryName = name.split('.').slice(0,-1)[0].split('/').slice(1,2)[0]; //produce font name from file name
...
elementLibrary.character.getLoadedFonts = function(){
    ...
    const defaultFontNames = ['defaultThick','defaultThin'];
    const loadedFontNames = fontFileNames.map(a => a.split('.').slice(0,-1)[0].split('/').slice(1,2)[0]).filter(name => elementLibrary.character.vectorLibrary[name].isLoaded);
...
elementLibrary.character.loadFont = function(fontName){
    ...
    const filename = elementLibrary.character.vectorLibrary[fontName].fileName;
