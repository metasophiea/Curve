    ...
    postMessage(
        {
            ...
            result: library.math.pathExtrapolation(
                event.data.path,
                event.data.thickness,
                event.data.capType,
                event.data.joinType,
                event.data.loopPath,
                event.data.detail,
                event.data.sharpLimit
            ),
        }
    )
};
        ...
        this.pathExtrapolation = function(path,thickness=10,capType='none',joinType='none',loopPath=false,detail=5,sharpLimit=thickness*4){
                    ...
                    for(var a = 0; a < jointData.length; a++){
                        //calculate segment angles    
                            if( a != jointData.length-1 ){
                                var tmp = library.math.getAngleOfTwoPoints( jointData[a].point, jointData[a+1].point );
            ...
            function path_to_rectangleSeries(path,thickness){
                ...
                for(var a = 1; a < path.length/2; a++){
                    var angle = library.math.getAngleOfTwoPoints( {x:path[a*2-2], y:path[a*2 -1]}, {x:path[a*2], y:path[a*2 +1]});
                    var left =  library.math.cartesianAngleAdjust(thickness, 0, Math.PI/2 + angle);
            ...
            function flatJoints(jointData,thickness){
                ...
                var perpenL = library.math.cartesianAngleAdjust(thickness, 0, jointData[0].departAngle - Math.PI/2);
                var perpenR = {x:-perpenL.x, y:-perpenL.y};
                for(var a = 1; a < jointData.length-1; a++){
                    var last_perpenL = perpenL;
                    var last_perpenR = perpenR;
                    var perpenL = library.math.cartesianAngleAdjust(thickness, 0, jointData[a].departAngle - Math.PI/2);
                    var perpenR = {x:-perpenL.x, y:-perpenL.y};
                ...
                for(var a = 1; a < jointData.length-1; a++){
                    ...
                    if(jointData[a].joiningAngle == Math.PI){
                        ...
                        for(var b = 1; b < partialDetail; b++){
                            ...
                            var p = library.math.cartesianAngleAdjust(last_perpenR.x, last_perpenR.y, -angle);
                        ...
                        for(var b = 1; b < partialDetail; b++){
                            var angle = b*(gapSize/partialDetail);
                            var p = library.math.cartesianAngleAdjust(last_perpenL.x, last_perpenL.y, -angle);
                            newPolygon.push( {x:jointData[a].point.x + p.x, y:jointData[a].point.y + p.y} );
                        }
                ...
                for(var a = 1; a < jointData.length-1; a++){
                    ...
                    if(jointData[a].joiningAngle == Math.PI){
                        ...
                        if( Math.abs(jointData[a].wingWidth) <= sharpLimit ){
                            var plus = library.math.cartesianAngleAdjust(0, jointData[a].wingWidth, Math.PI/2 + jointData[a].wingAngle);
                            ...
                            var partialWingA = library.math.cartesianAngleAdjust(0, -length, Math.PI/2 + jointData[a].implementAngle);
                            var partialWingB = library.math.cartesianAngleAdjust(0, length, Math.PI/2 + jointData[a].departAngle);
            ...
            function roundCaps(jointData,thickness,detail=5){
                ...
                //top
                    ...
                    for(var a = 0; a < detail+1; a++){
                        var p = library.math.cartesianAngleAdjust(thickness, 0, jointData[0].departAngle + Math.PI/2 + (a/(detail))*(Math.PI) );
                ...
                //bottom
                    ...
                    for(var a = 0; a < detail+1; a++){
                        var p = library.math.cartesianAngleAdjust(thickness, 0, jointData[jointData.length-1].implementAngle - Math.PI/2 + (a/(detail))*(Math.PI) );
        ...
        this.getTrianglesFromGlyphPath = function(glyphPath,detail=2){
            ...
            //convert glyphPath into segments with holes
                ...
                //reorder paths in order of size
                    paths = paths.map(a => {
                        var tmp = library.math.boundingBoxFromPoints(a); 
